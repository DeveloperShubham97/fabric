...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating chandigarh Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-chandigarh.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Gujrat Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-gujrat.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/chandigarh/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for chandigarh and Gujrat"
...
# call the script to create the channel, join the peers of chandigarh and gujrat,
...
function networkDown() {
  # stop org3 containers also in addition to chandigarh and gujrat, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/chandigarh/msp organizations/fabric-ca/chandigarh/tls-cert.pem organizations/fabric-ca/chandigarh/ca-cert.pem organizations/fabric-ca/chandigarh/IssuerPublicKey organizations/fabric-ca/chandigarh/IssuerRevocationPublicKey organizations/fabric-ca/chandigarh/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/gujrat/msp organizations/fabric-ca/gujrat/tls-cert.pem organizations/fabric-ca/gujrat/ca-cert.pem organizations/fabric-ca/gujrat/IssuerPublicKey organizations/fabric-ca/gujrat/IssuerRevocationPublicKey organizations/fabric-ca/gujrat/fabric-ca-server.db'
